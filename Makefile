...
$(OUT_DIR)/$(DOCKER_BUILDER): $(GO_SRC) $(BPF_SRC) $(MAKEFILE_LIST) Dockerfile | $(OUT_DIR)
	$(CMD_DOCKER) build -f Dockerfile.builder -t $(DOCKER_BUILDER) --iidfile $(OUT_DIR)/$(DOCKER_BUILDER) --target builder --output "type=image,push=false" .
...
.PHONY: docker
docker:
	$(CMD_DOCKER) build --build-arg VERSION=$(VERSION) -t $(OUT_DOCKER) :latest --output "type=image,push=false" .
