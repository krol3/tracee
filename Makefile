...
$(OUT_DIR)/$(DOCKER_BUILDER): $(GO_SRC) $(BPF_SRC) $(MAKEFILE_LIST) Dockerfile | $(OUT_DIR)
	$(CMD_DOCKER) buildx build -f Dockerfile.builder -t $(DOCKER_BUILDER) --iidfile $(OUT_DIR)/$(DOCKER_BUILDER) --target builder .
...
.PHONY: docker
docker:
	$(CMD_DOCKER) buildx build --build-arg VERSION=$(VERSION) -t $(OUT_DOCKER):latest .
